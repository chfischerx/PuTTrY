...
import { createSession, getAllSessions, getSession, renameSession, killSession, getSessionProcessInfo } from "../sessions/pty-manager.js"
import { parseGuestSessionToken } from "../sessions/guest-store.js"
import logger from "../lib/logger.js"

function isGuest(req: Request): boolean {
  return !!parseGuestSessionToken(req)
}
...
export function createTerminalRouter(): Router {
  ...
  router.post("/", (req: Request, res: Response) => {
    if (isGuest(req)) {
      res.status(403).json({ error: "Guests cannot create sessions" })
      return
    }
    try {
  ...
  router.patch("/:id", (req: Request<{ id: string }, unknown, { label?: string }>, res: Response) => {
    if (isGuest(req)) {
      res.status(403).json({ error: "Guests cannot rename sessions" })
      return
    }
    try {
      const { id } = req.params
  ...
  router.delete("/:id", (req: Request<{ id: string }>, res: Response) => {
    if (isGuest(req)) {
      res.status(403).json({ error: "Guests cannot delete sessions" })
      return
    }
    try {
      const { id } = req.params
