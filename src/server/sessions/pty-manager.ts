...
import { createLockRequest, setGuestClientId } from "./guest-store.js"
...
export function attachWebSocket(
  ...
  clientId: string = "",
  browserToken: string = "",
  clientContext?: { isGuest?: boolean; guestName?: string; guestSessionId?: string }
): boolean {
  ...
  const isGuest = clientContext?.isGuest ?? false
  const guestName = clientContext?.guestName ?? ""
  const guestSessionId = clientContext?.guestSessionId ?? ""
  ...
  // Track guest presence and store clientId
  if (isGuest && guestName) {
    broadcastSync({ type: "guest-viewer-added", sessionId: id, guestName, clientId })
    if (guestSessionId) {
      logger.info(`[guest] Setting clientId for guest session ${guestSessionId.slice(-8)}: ${clientId}`)
      setGuestClientId(guestSessionId, clientId)
    } else {
      logger.warn(`[guest] guestSessionId not provided for guest ${guestName}`)
    }
  }
  ...
  ws.on("message", (data) => {
    ...
    if (dataStr.startsWith('{')) {
      try {
        ...
        if (message.type === "resize") {
          ...
          session.cols = cols
          session.rows = rows
        } else if (message.type === "request-lock") {
          // Guest requesting control
          if (isGuest && guestName) {
            const request = createLockRequest(id, guestName, clientId, () => {
              // Timeout callback: notify frontend of expiry
              broadcastSync({ type: "lock-request-expired", requestId: request.id })
              ws.send(JSON.stringify({ type: "lock-request-expired" }))
            })
            broadcastSync({ type: "lock-request", requestId: request.id, sessionId: id, guestName })
            logger.info(`[pty-manager] Lock request created: ${request.id} for session ${id} by guest ${guestName}`)
          }
    ...
    // Guest clients cannot send keystrokes unless they hold the lock
    if (!isGuest && session.inputLockClientId === clientId) {
    ...
    } else if (isGuest && session.inputLockClientId === clientId) {
      // Guest holds the lock, allow keystrokes
      const input = dataStr
      if (input.length > MAX_PTY_INPUT_SIZE) {
        logger.warn(`[pty-manager] Input exceeds maximum size (${input.length} > ${MAX_PTY_INPUT_SIZE}), truncating`)
        session.pty.write(input.slice(0, MAX_PTY_INPUT_SIZE))
  ...
  ws.on("close", () => {
    ...
    // Notify if guest disconnected
    if (isGuest && guestName) {
      broadcastSync({ type: "guest-viewer-removed", sessionId: id, guestName, clientId })
    }
...
export function approveGuestLockRequest(sessionId: string, guestClientId: string, requestId?: string): void {
  const session = sessions.get(sessionId)
  if (!session) {
    logger.warn(`[pty-manager] Cannot approve lock: session ${sessionId} not found`)
    return
  }

  session.inputLockClientId = guestClientId
  broadcastSync({ type: "input-lock-acquired", sessionId, clientId: guestClientId })
  if (requestId) {
    broadcastSync({ type: "lock-request-resolved", requestId, approved: true })
  }
  logger.info(`[pty-manager] Guest lock approved for session ${sessionId}`)
}

export function denyGuestLockRequest(sessionId: string, requestId: string): void {
  broadcastSync({ type: "lock-request-resolved", requestId, approved: false })
  logger.info(`[pty-manager] Guest lock denied for session ${sessionId}`)
}
